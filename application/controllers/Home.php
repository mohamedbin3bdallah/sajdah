...
class Home extends CI_Controller {
	...
	public function index()
	{
		if($this->session->userdata('uid') != FALSE)
		{
			...
			$uprepare = $this->Admin_mo->rate('users.uname as name,COUNT(logsystem.id) as count','logsystem','LEFT OUTER JOIN users ON users.uid = logsystem.user GROUP BY logsystem.user ORDER BY count DESC limit 5');
